...
def compare_faces(face1_path, face2_path, threshold=THRESHOLD_FOR_COMPARE_FACES):
    """
    Compare two faces to determine if they are the same using face_recognition library.

    Args:
        face1_path (str): Path to the first face image.
        face2_path (str): Path to the second face image.
        threshold (float): Threshold for the face recognition comparison.

    Returns:
        dict: A dictionary containing whether the faces are the same or not.
    """
    try:
        # Load the images
        image1 = face_recognition.load_image_file(face1_path)
        image2 = face_recognition.load_image_file(face2_path)
        ...
        is_match = bool(results[0] and distance < threshold)

        response = {
            'distance': float(distance),
            'accuracy': float(1 - distance),
