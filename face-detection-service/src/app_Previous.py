...
def extract_images_from_pdf(pdf_path):
    ...
    Returns:
        list: A list of dictionaries containing image data and their extensions.
    """
    try:
        ...
        for i, img in enumerate(images):
            ...
            image_data.append({
                'image': img_np,
                'ext': 'png'  # pdf2image outputs images in PNG format
            })
        return image_data
    except Exception as e:
        logging.error(f"Error extracting images from PDF: {e}")
...
def process_media():
    ...
    if file1.filename.lower().endswith(PDF_EXTENSIONS) and file2.filename.lower().endswith(VIDEO_EXTENSIONS):
        images = extract_images_from_pdf(file1_path)
        if images is None or len(images) == 0:
            return jsonify({'error': 'No images found in the PDF file'}), 400
        
        image_paths = save_extracted_images(file1_path, images, output_folder)
        ...
        # if not is_image_clear(video_image_path):
        #     return jsonify({'error': 'The extracted image from the video is not clear. Please upload a clearer video.'}), 400
