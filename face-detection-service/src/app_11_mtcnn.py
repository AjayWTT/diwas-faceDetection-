...
def compare_faces(face1_path, face2_path, threshold=0.9, model_name=MODEL_NAME_FOR_COMPARE_FACES, distance_metric=DISTANCE_METRIC_FOR_COMPARE_FACES):
    try:
        result = DeepFace.verify(img1_path=face1_path, img2_path=face2_path, model_name=model_name, distance_metric='euclidean_l2', align=True, detector_backend='mtcnn')
...
def process_media():
    ...
    #                    [-1, 5,-1], 
    #                    [0, -1, 0]])
    # sharpened_image = cv2.filter2D(normalized_image, -1, kernel)

    # # Resize the image
    # resized_image = cv2.resize(sharpened_image, (224, 224))
    # # kernel = np.array([[0, -1, 0], [-1, 5,-1], [0, -1, 0]])
    # # image = cv2.filter2D(src=normalized_image, ddepth=-1, kernel=kernel)
    # cv2.imwrite(file1_path, resized_image)
    ...
    if file1_path.lower().endswith(PDF_EXTENSIONS) and file2_path.lower().endswith(VIDEO_EXTENSIONS):
        ...
        cv2.imwrite(face1_path, faces_from_pdf[0])
        cv2.imwrite(face2_path, faces_from_video[0])
